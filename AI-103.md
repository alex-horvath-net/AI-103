...
`AzureKeyVault` identifies the connection target as Key Vault `KV1`. `AccountManagedIdentity` provides keyless authentication through the Foundry account or project managed identity; that identity must have the required Key Vault RBAC permissions.
