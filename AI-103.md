...
|---|---|---|
| **Keyword** | Exact words, names, or IDs matter | Lexical matching |
| **Vector** | Meaning or similarity matters | Uses embeddings |
| **Hybrid** | Exact terms and meaning both matter | Combines keyword and vector results |
| **Semantic ranker** | Results need better ordering | Reranks; it is not an index |
| **Agentic retrieval** | Complex questions need decomposition and citations | An LLM plans subqueries; adds cost and latency |

A strong classic-RAG baseline is **hybrid search plus semantic ranking**.

#### Indexing flow

**Data source → indexer → optional skillset/chunking/vectorisation → index → query**

- The **indexer** imports and refreshes data.
- A **skillset** enriches or transforms it.
- The **index** stores searchable fields and vectors.
- Use the same embedding model for indexed and query vectors.
- Use filters for security, tenant, category, or date constraints.

Choose **classic RAG** for simplicity, speed, and application-controlled retrieval. Choose **agentic retrieval** for complex conversational retrieval and query planning; verify current GA/preview status.

#### Exam traps

- Semantic ranking is not vector search.
- Vector search is not complete RAG; a model must use the retrieved evidence.
- Hybrid runs keyword and vector retrieval together.
- Chunk long documents before embedding.
- Embeddings are not readable answers.

#### Exam summary

**Exact → keyword. Meaning → vector. Both → hybrid. Reorder → semantic ranker. Plan complex retrieval → agentic retrieval.**
