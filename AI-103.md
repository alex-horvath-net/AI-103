...
| Requirement | Choose |
|---|---|
| Generate or reason | **Foundry model** |
| Retrieve private, current, or citable data | **RAG with Azure AI Search** |
| Reusable permission-aware agent knowledge | **Foundry IQ** |
| Speech, translation, extraction, or vision | Appropriate **Foundry Tool** |
| Select tools or perform multiple steps | **Foundry Agent Service** |
| Reason across supported media | **Multimodal model** |

**Memorise:** models generate; Search retrieves; Foundry IQ supplies managed knowledge; Tools specialise; agents orchestrate.

#### Grounding

**Data → chunk/embed/index → retrieve → add evidence to prompt → generate**

Use RAG when information is private, changing, too large for the prompt, or needs citations. RAG adds knowledge at inference time; it does not retrain the model.

Use **Content Understanding** for schema-based extraction from documents, images, audio, or video. Use **Speech**, **Translator**, or **Vision** when explicitly required.

#### Exam traps

- A Foundry project organises assets; it is not a search engine.
- Search retrieves evidence; it does not generate the answer.
- Foundry IQ uses Azure AI Search agentic retrieval.
- A single model call does not require an agent.
- Verify current preview, feature, and regional support.

#### Exam summary

**Generate → model. Retrieve → Search. Agent knowledge → Foundry IQ. Specialised task → Tool. Multistep action → agent.**
