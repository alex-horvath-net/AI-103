...
| Capability | Exam meaning | Typical use |
|---|---|---|
| **LLM** | Large model for stronger language generation and complex reasoning | Generation, difficult summarisation, complex extraction, coding |
| **SLM** | Smaller model optimised for simpler, high-volume tasks | Classification, routing, simple extraction or summarisation |
| **Multimodal model** | Reasons across supported text, image, audio, or video inputs | Answer questions about an image or combine text with media |
| **Embedding model** | Converts content into vectors; it does **not** generate answers | Semantic search, similarity, clustering, RAG retrieval |
| **Image/video model** | Generates or edits visual media | Create an image or video |
| **Foundry Tool** | Performs a specialised, bounded capability | Speech, Translator, Content Understanding, Vision |
| **Agent** | Orchestrates a model, instructions, knowledge, and tools | Multistep work, external data access, tool calls and actions |

**Memorise:** the model reasons or generates; a Foundry Tool performs a specialised operation; an agent coordinates models, knowledge, and tools.
...
3. Prefer a **specialised tool** for a clearly bounded requirement.
4. Add **Azure AI Search and embeddings** when private, current, or citable knowledge must be retrieved.
5. Use an **agent** only when the solution must select tools, access external systems, or perform multiple steps.
6. Compare quality, latency, throughput, context size, cost, safety, region, inferencing residency, and deployment constraints.
7. Select the **smallest model that passes evaluation with representative data**.

#### Exam scenario mapping

| Requirement | Best starting choice |
|---|---|
| Complex generation, reasoning, or summarisation | LLM |
| High-volume classification or simple extraction | SLM |
| Open-ended reasoning about an image or other supported media | Multimodal model |
| Semantic retrieval or RAG | Embedding model + Azure AI Search; add an LLM/SLM to generate the answer |
| Structured extraction from documents, images, audio, or video | Content Understanding |
| Speech recognition, speech synthesis, or speech translation | Speech |
| Text or document translation | Translator |
| OCR or another explicit visual-analysis feature | Currently supported Vision or Content Understanding capability |
| Generate or edit an image/video | Image/video generation model |
| Call tools and complete a multistep process | Agent + model + authorised tools |

#### Essential trade-offs

| Choice | Main advantage | Main disadvantage |
|---|---|---|
| **LLM** | Better for complex, ambiguous reasoning | Usually higher cost and latency |
| **SLM** | Lower cost, lower latency, higher throughput | Less capable on complex or unfamiliar tasks |
| **Multimodal model** | Flexible reasoning across media | Less predictable than a bounded specialised API |
| **Specialised tool** | Predictable interface for a defined task | Narrower capability |
| **Managed deployment** | Less infrastructure and operational work | Deployment options, quota, region, and cost still constrain the solution |
| **Self-hosted model** | Greater hosting control | You manage capacity, scaling, security, updates, and monitoring |

Do not select by model size alone. Use Foundry benchmarks to shortlist candidates, then evaluate them against the real workload.

#### Common exam traps

- An **agent is not a model** and is unnecessary for a simple prompt-response application.
- An **embedding model does not produce the final natural-language answer**.
- **RAG** requires retrieval; choosing a larger LLM does not give it access to private or current data.
- **Multimodal understanding** and **image/video generation** are different capabilities.
- A file does not automatically require a multimodal model; extracted plain text can use a language model.
- Prefer Speech, Translator, Content Understanding, Vision, or Search when the question explicitly requires their bounded capability.
- The largest model is not automatically best; choose the least costly option that meets quality and safety requirements.
- **Inferencing residency** means where prompts and outputs are processed during inference. It is different from data-at-rest location and depends on deployment type.
- Model features, lifecycle, quota, deployment type, and regional availability change. Verify the current model catalogue, model card, and region documentation.

#### Exam summary

**LLM/SLM:** reason and generate.  
**Multimodal model:** reasons across supported media.  
**Embedding model:** creates vectors for retrieval.  
**Image/video model:** generates media.  
**Foundry Tool:** performs a specialised task.  
**Azure AI Search:** retrieves knowledge.  
**Agent:** orchestrates models, knowledge, and tools.
...
| Requirement | Choose |
|---|---|
| Self-contained generation or reasoning | **Foundry model** |
| Ground an answer in private, current, or citable data | **RAG** |
| Control indexes, fields, filters, keyword/vector/hybrid queries | **Azure AI Search** |
| Give agents reusable, permission-aware knowledge | **Foundry IQ knowledge base** |
| Speech, translation, structured content processing, or vision | Appropriate **Foundry Tool** |
| Select tools, maintain state, or perform multiple actions | **Foundry Agent Service** |
| Reason across text and supported images/audio/video | **Multimodal model or Content Understanding**, depending on the task |

**Memorise:** models generate; Search retrieves; Foundry IQ provides managed agent knowledge; Tools perform specialised capabilities; agents orchestrate.

#### Grounding flow

**Data → parse/chunk → embed/index → retrieve → add evidence to prompt → model generates a grounded answer**

Use RAG when knowledge is private, too large for the prompt, changes frequently, or needs citations. RAG supplies context at inference time; it does **not** retrain the model.

#### Multimodal decision

- Use a **multimodal model** for flexible reasoning across media.
- Use **Content Understanding** for schema-based extraction or classification from documents, images, audio, or video.
- Use **Speech**, **Translator**, or **Vision** when the requirement explicitly matches that bounded capability.

#### Exam traps

- A Foundry project organises assets; it is not a search engine.
- Azure AI Search retrieves content; it does not generate the final answer.
- Foundry IQ uses Azure AI Search agentic retrieval to expose managed knowledge to agents.
- Do not choose an agent for a single model call with no tools or multistep behaviour.
- Verify current feature, preview, model, and regional support.

#### Exam summary

Choose the narrowest capability that fills the gap:

**generate → model; retrieve → Azure AI Search; managed agent knowledge → Foundry IQ; specialised processing → Foundry Tool; multistep action → agent.**
...
| Method | Choose when | Exam distinction |
|---|---|---|
| **Keyword/full-text** | Exact words, names, IDs, or terminology matter | Lexical matching |
| **Vector** | Meaning or conceptual similarity matters | Uses embeddings and nearest-neighbour search |
| **Hybrid** | Exact and conceptual matches both matter | Runs keyword and vector search together and merges results |
| **Semantic ranker** | Initial text or hybrid results need better ordering | Reranks results; it is not an index |
| **Agentic retrieval** | Complex conversational questions need decomposition and citations | An LLM plans subqueries; usually adds cost and latency |

A strong classic-RAG starting point is **hybrid search plus semantic ranking**.

#### Indexing flow

**Data source → indexer → optional skillset/chunking/vectorisation → search index → query**

- **Indexer:** pulls data and schedules refresh.
- **Skillset:** enriches or transforms content.
- **Index:** stores searchable fields and vectors.
- **Vectoriser/embedding model:** converts text or images into vectors.
- Use the **same embedding model** for indexed content and query vectors.
- Apply filters for tenant, security, category, date, or other exact constraints.

#### Classic or agentic retrieval

- Choose **classic RAG** for simplicity, speed, generally available features, and application-controlled orchestration.
- Choose **agentic retrieval** for complex conversational queries, query planning, multiple knowledge sources, and structured citations. Check current GA/preview status.

#### Exam traps

- Semantic ranking is not vector search.
- Vector search is not complete RAG; RAG also supplies retrieved evidence to a generative model.
- Hybrid search is one query combining lexical and vector retrieval.
- Chunk long documents before embedding so relevant passages can be retrieved independently.
- Embeddings are not human-readable answers.

#### Exam summary

**Exact terms → keyword. Meaning → vector. Both → hybrid. Better ordering → semantic ranker. Complex planned retrieval → agentic retrieval.**
...
| Component | Purpose |
|---|---|
| **Conversation state** | Preserves recent turns and execution context |
| **Memory** | Retains useful information beyond the immediate conversation |
| **Knowledge** | Grounds answers in external facts or documents |
| **Tool** | Retrieves data, calculates, or performs an action |

**Memorise:** state tracks the interaction; memory retains information; knowledge supplies facts; tools do work.

#### Choose the integration

| Requirement | Choose |
|---|---|
| Search uploaded documents | **File search** |
| Search an existing enterprise index | **Azure AI Search tool** |
| Reusable permission-aware agent knowledge | **Foundry IQ knowledge base** |
| Current public information | **Web search** |
| Sandboxed calculations or file processing | **Code interpreter** |
| Call application code executed by your client | **Function calling** |
| Call an API described by an OpenAPI document | **OpenAPI tool** |
| Discover and call remote standardised tools | **MCP** |
| Manage a large reusable tool collection | **Toolbox/tool search**, where supported |

Function calling means the **model requests a function call**; the application executes it and returns the result. MCP standardises tool discovery and invocation; it does not replace the agent or model.

#### Security rules

- Give the agent only the tools and permissions required.
- Enforce authentication and authorisation in the target service; model tool selection is not authorisation.
- Require approval for destructive, financial, or otherwise consequential actions.
- Treat retrieved content and tool output as untrusted input because they can contain indirect prompt injection.
- Trace tool choice, arguments, results, failures, state, and approvals.

#### Exam traps

- Knowledge retrieval does not retrain the model.
- State, memory, knowledge, and tools are not interchangeable.
- File search is for uploaded documents; Azure AI Search is for an existing controlled search solution.
- Adding more tools can reduce selection reliability and enlarge the security boundary.
- A tool description and schema guide selection but do not grant permission.

#### Exam summary

**Remember conversation → state/memory. Ground facts → knowledge/search. Calculate or act → tool. Coordinate them → agent.**

#### Sources

- [Foundry Agent Service overview](https://learn.microsoft.com/en-us/azure/foundry/agents/overview)
- [File search](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/file-search)
- [Foundry IQ connection](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/foundry-iq-connect)
- [MCP tools](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/model-context-protocol)
- [Toolboxes](https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/toolbox-overview)
