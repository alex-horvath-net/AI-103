...
| Component | Purpose |
|---|---|
| **State** | Preserves conversation context |
| **Memory** | Retains information beyond immediate turns |
| **Knowledge** | Grounds answers in external facts |
| **Tool** | Retrieves, calculates, or acts |

**Memorise:** state tracks; memory retains; knowledge grounds; tools act.
...
| Existing search index | **Azure AI Search tool** |
| Managed permission-aware knowledge | **Foundry IQ** |
| Current public information | **Web search** |
| Sandboxed calculations/files | **Code interpreter** |
| Client-executed code | **Function calling** |
| API described by OpenAPI | **OpenAPI tool** |
| Standardised remote tools | **MCP** |

Function calling: the model requests a call; the application executes it. MCP standardises tool discovery/invocation; it does not replace the model or agent.

#### Security and exam traps

- Grant only required tools and permissions.
- Authorise in the target service; model selection is not authorisation.
- Require approval for consequential actions.
- Treat retrieved/tool content as untrusted because of prompt injection.
- Knowledge retrieval does not retrain the model.
- State, memory, knowledge, and tools are different.
- File search handles uploaded files; Azure AI Search provides controlled enterprise retrieval.

#### Exam summary

**Remember → state/memory. Ground → knowledge/search. Calculate or act → tool. Coordinate → agent.**
...
- [Foundry IQ](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/foundry-iq-connect)
- [MCP](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/model-context-protocol)
