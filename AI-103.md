...
Capacity management answers four separate questions:

1. **Can the deployment be created?** Check model availability, region, deployment type, quota and actual capacity.
2. **Can it admit the workload?** Check TPM, RPM and application concurrency.
3. **Can it serve predictably?** Choose shared Standard, reserved Provisioned or asynchronous Batch capacity.
4. **What will it cost?** Compare pay-per-token usage with reserved throughput or provisioned compute.

Do not use **quota**, **capacity**, **rate limit** and **billing** as if they were synonyms.

#### Key terms

| Term | Meaning | Exam implication |
|---|---|---|
| **Quota** | An Azure allowance or policy limit that can be allocated to deployments | Quota is not the same as actual available processing capacity |
| **Capacity** | The processing capacity currently available for a model/version in a region or scope | PTU quota does not guarantee that the requested deployment capacity is available |
| **Allocation** | The amount of quota assigned to one deployment | Assigned but idle quota remains unavailable to other deployments |
| **TPM** | Tokens per minute | The main allocation and throughput unit for Standard deployments |
| **RPM** | Requests per minute | A workload can hit RPM even when it has unused TPM |
| **Concurrency** | Requests being processed at the same time | Control it in the application with worker limits and back-pressure |
| **PTU** | Provisioned throughput unit | The capacity unit for Provisioned deployments |
| **HTTP 429** | The request was throttled or rate-limited | Honour retry information and reduce or reshape demand |
| **Billing** | The charge basis for the chosen deployment | Usage-based token billing differs from reserved-capacity or compute billing |

#### Quota and capacity

Quota scope is not one universal rule. It depends on the model and offering. The Foundry quota view identifies whether a pool is global, data-zone or regional, and some models use shared subscription-level pools while others are scoped more narrowly.

Therefore, in an exam scenario:

- Check the model's availability in the required region.
- Check the deployment type and its applicable quota pool.
- Check the quota **Scope**, not only the resource location.
- Check whether actual model capacity is available.
- Do not assume that quota in one region or deployment type can be used by another.
- Do not assume that creating a new project or renaming a deployment creates more quota.
- Remember that quota assigned to an idle deployment is still allocated.

For Standard deployments, quota is normally managed as a TPM allocation. For Provisioned deployments, quota is managed as PTUs. PTU-to-throughput results depend on the model and workload shape.

**Important distinction:** quota is an Azure allowance; capacity is the processing resource that is actually available. Having enough PTU quota does not guarantee that the requested model capacity is available in the target region.

#### TPM, RPM and admission control

A deployment can be limited by different controls:

- **TPM limit:** the token volume is too high.
- **RPM limit:** too many requests arrive, even if each request is small.
- **Concurrency limit:** too many requests are in flight at the same time.
- **Shared-capacity pressure:** a Standard deployment may be throttled when shared capacity is constrained.

The admission-time token estimate is not always equal to the final billed token count. Prompt tokens, retrieved context, tool messages and the configured maximum output can affect the capacity estimate. A model may return fewer output tokens than the configured maximum, but the request can still be admitted using the larger estimate.

Use the Retry-After value when supplied. For transient 429, 5xx or timeout failures, use bounded exponential back-off with jitter. An immediate retry sends another request while the system is already overloaded and can amplify throttling.

#### Capacity estimation

Required TPM ≈ peak requests per minute × tokens per request

Required RPM ≈ peak requests per minute

Required concurrency ≈ requests per second × average latency in seconds

Count the complete request shape:

- Input prompt and conversation history
- Retrieved grounding context
- Tool definitions and tool results
- Expected output
- Configured maximum output for safe admission planning
- Peak traffic, not only the daily average
- A safety margin for bursts and latency variation

**Example:** 120 requests per minute at approximately 2,000 tokens per request requires about 240,000 TPM. If average latency is two seconds, the application must also be prepared for roughly four in-flight requests at the average rate.

Benchmark representative traffic where possible. The same model and TPM can behave differently when requests have different prompt sizes, output sizes, cache-hit rates and concurrency.

#### Choose the deployment for the workload

| Requirement | Best starting choice | Capacity and cost model |
|---|---|---|
| Variable, intermittent or lower-volume real-time demand | **Standard** | Shared capacity; usage-based token billing |
| Sustained, predictable and latency-sensitive demand | **Provisioned** | Reserved PTUs; billed for deployed capacity rather than only successful token usage |
| Large non-interactive workload | **Batch** | Asynchronous processing; suitable when immediate responses are unnecessary |
| Open or custom model requiring dedicated managed accelerators | **Managed compute** | Provisioned accelerator/compute time; you control the managed compute configuration |

**Exam rule:** more TPM on a Standard deployment gives it more admission allowance; it does not turn shared Standard capacity into reserved Provisioned capacity.

#### Application scaling and back-pressure

Application instances and model capacity are different layers:

Many application instances → one model deployment → the same model limits

Adding application instances helps when the application itself is the bottleneck—for example, HTTP handling, CPU, memory, connection handling or background-worker capacity. It does not automatically increase the model deployment's TPM or RPM.

If the model is the bottleneck, extra application instances can create more simultaneous calls and produce more 429 responses.

Use back-pressure to protect the downstream model:

1. Accept work only while the application has bounded capacity.
2. Place excess work in a bounded queue or reject it clearly.
3. Limit the number of model calls in flight.
4. Retry transient failures with delay and jitter.
5. Shed or defer non-critical work when the queue is full.

In .NET, a bounded Channel<T> or a controlled SemaphoreSlim can implement this pattern. The important exam concept is not the class name; it is limiting admission before the downstream service is overloaded.

#### Agent-specific limits

An agent workload can encounter two independent limit layers:

1. **Agent Service limits** for artefacts such as files, threads, messages, vector-store attachments or registered tools.
2. **Model-deployment limits** for TPM, RPM, concurrency and shared capacity.

Increasing model quota does not remove an Agent Service artefact limit. Increasing Agent Service storage or file limits does not increase model throughput. Diagnose the layer that returned the error.

#### Cost footprint and optimisation

| Deployment or activity | Main cost consideration |
|---|---|
| Standard | Input/output token usage and repeated calls |
| Provisioned | Deployed PTUs, including capacity that is under-utilised |
| Batch | Efficient for high-volume asynchronous work |
| Managed compute | Provisioned accelerator or VM/GPU time |
| Retrieval | Search replicas, partitions and query volume |
| Evaluation | Number of evaluator/model calls and dataset size |

Reduce cost in this order:

- Use the smallest model that meets the quality requirement.
- Remove unnecessary history and retrieved context.
- Limit output length appropriately.
- Improve prompts and tool schemas so the agent needs fewer calls.
- Use caching only when freshness, authorisation and tenant isolation are safe.
- Use Batch for offline work.
- Reduce unnecessary evaluator calls while retaining meaningful coverage.
- Right-size Search replicas and partitions.
- Autoscale application/runtime resources based on measured demand.
- Add budgets, tags and Cost Management alerts.

Optimise **cost per successful, acceptable task**, not only cost per individual call. A cheap call that produces an unusable answer and causes retries can cost more overall.

#### Monitoring and corrective action

Monitor both usage and outcomes:

| Area | Signals |
|---|---|
| Capacity | TPM, RPM, concurrency and queue depth |
| Reliability | 429, 5xx, timeouts, retries and rejected work |
| Performance | Latency, time to first token, tokens per second and throughput |
| Quality | Groundedness, relevance, task adherence, safety and tool success |
| Cost | Input tokens, output tokens, evaluator calls and total spend |

Use the signal to choose the correction:

- **429 from TPM/RPM:** reduce demand, reallocate quota, request quota or use a suitable Provisioned deployment.
- **Queue growth:** reduce admission or increase the capacity of the actual bottleneck.
- **High application CPU/memory:** add application instances or right-size the runtime.
- **High latency variation:** reduce request size or consider Provisioned throughput.
- **High cost:** reduce tokens, calls or model size.
- **Quality regression:** investigate the model, prompt, retrieval or tool version and roll back when necessary.

#### Repeatable exam workflow

1. Identify whether the workload is real-time or asynchronous.
2. Estimate peak RPM, TPM, latency and concurrency.
3. Identify the bottleneck layer: application, model, Agent Service, Search or downstream tool.
4. Verify model, region, deployment-type, quota and capacity availability.
5. Choose Standard, Provisioned, Batch or managed compute.
6. Allocate quota or PTUs to the deployment.
7. Set application concurrency and back-pressure.
8. Implement bounded retries using Retry-After, exponential back-off and jitter.
9. Monitor usage, throttling, latency, quality and cost.
10. Reallocate, resize, optimise or roll back based on measured evidence.

#### High-value exam traps

- **Quota is not capacity:** quota may permit a deployment, but actual model capacity may still be unavailable.
- **TPM is not RPM:** many small requests can hit RPM first; a few large requests can hit TPM first.
- **Application scaling is not model scaling:** more app replicas do not increase one model deployment's TPM.
- **More quota is not reserved capacity:** Standard remains shared unless the deployment type changes.
- **Instant retries make throttling worse:** use delay, jitter and bounded retry counts.
- **A lower actual response size does not erase a high configured maximum-output estimate.**
- **Idle allocations still consume quota:** quota is assigned capacity, not only active traffic.
- **A new project or deployment name does not create quota.**
- **Batch is asynchronous:** it is not the answer for interactive chat requiring an immediate response.
- **Agent limits and model limits are separate:** fix the layer that produced the error.
- **PTU quota does not guarantee regional capacity.**
- **The resource location alone does not prove the inference or quota scope.**
- **The cheapest call is not necessarily the cheapest successful solution.**
...
429 = throttled; slow down and retry correctly

Quota = allocatable allowance

Capacity = processing resource currently available

Standard = shared, usage-based tokens

Provisioned = reserved PTUs, predictable throughput

Batch = asynchronous high-volume work

Managed compute = dedicated managed accelerator time

Back-pressure = limit incoming work before the downstream model overloads

Agent workload = Agent Service limits + underlying model limits

**Best exam rule:** identify the bottleneck and workload shape first; then verify quota and capacity, choose the deployment type, control concurrency, handle throttling and optimise cost per successful task.

#### Sources

- [Foundry Models quotas and limits](https://learn.microsoft.com/en-us/azure/foundry/foundry-models/quotas-limits)
- [Manage and increase quotas for Foundry projects](https://learn.microsoft.com/en-us/azure/foundry/how-to/quota)
...
- [Performance and latency](https://learn.microsoft.com/en-us/azure/foundry/openai/how-to/latency)
- [Foundry Agent Service limits, quotas and regional support](https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/limits-quotas-regions)
#### Quize
