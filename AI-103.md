...
Capacity problems can occur at different layers:

1. **Application layer:** CPU, memory, threads, connections and queues.
2. **Agent layer:** threads, tool calls, runs, concurrency and service limits.
3. **Model deployment:** TPM, RPM, concurrency or provisioned capacity.
4. **Supporting services:** Azure AI Search, storage, APIs and tools.

Always identify the saturated layer before scaling anything.

#### 1. TPM versus RPM

| Limit | Meaning | Usually exhausted by |
|---|---|---|
| **TPM** | Tokens per minute | Large prompts, retrieved context, conversation history and long outputs |
| **RPM** | Requests per minute | Many small requests |
| **Concurrency** | Simultaneous active requests | Long-running requests or traffic bursts |

A deployment can remain below its TPM limit while hitting RPM, or vice versa.

Examples:

- Thirty large prompts per minute can exhaust **TPM** without exhausting RPM.
- One thousand very small requests can exhaust **RPM** while token usage remains low.

Azure can use an estimated token count for admission and rate limiting. This can include prompt tokens and the configured maximum output. Billing uses actual billable usage, so the rate-limit estimate and billed token count are not necessarily identical. Exact ratios and calculations depend on the model and deployment type. See [Microsoft quota and limits documentation](https://learn.microsoft.com/en-us/azure/foundry/openai/quotas-limits).

> **Exam rule:** inspect both TPM and RPM. Never assume every HTTP 429 is caused by TPM.

#### 2. Quota, allocation and capacity

These terms are different:

| Term | Meaning |
|---|---|
| **Quota** | Maximum capacity Azure permits the subscription to allocate |
| **Allocation** | Part of that quota assigned to a deployment |
| **Usage** | Capacity actually consumed by requests |
| **Physical capacity** | Infrastructure currently available for deployment |
...
Reducing Deployment A’s allocation can free quota for Deployment B. An idle deployment can still hold allocated quota.

Creating another project or renaming a deployment does not automatically create more quota.

##### Current quota-scope nuance

Do not memorise one universal scope. Scope depends on the model and deployment type.

Traditional scopes can include:
...
Newer subscription-level quota management is being introduced for supported models. For example, Global Standard deployments of the same model and version can share a subscription-level pool across regions, while Data Zone Standard can use a data-zone pool.

> **Exam rule:** use the scenario’s stated scope and verify the current Quota pane. Never assume quota can move between incompatible regions, deployment types, models or data zones.

See [Manage Foundry quota](https://learn.microsoft.com/en-us/azure/foundry/how-to/quota).

#### 3. Standard versus Provisioned

| Requirement | Best starting choice |
|---|---|
| Variable or intermittent demand | **Standard** |
| Shared capacity and usage-based billing | **Standard** |
| Sustained, predictable high demand | **Provisioned** |
| Predictable throughput and reduced latency variation | **Provisioned** |
| Large non-interactive workload | **Batch** |

##### Standard

- Shared model infrastructure
- Generally usage-based, pay-per-token billing
- Capacity controlled through applicable TPM/RPM quota
- Suitable for variable traffic
- Can experience throttling or latency variation

Increasing Standard TPM increases admission capacity. It does not turn shared capacity into reserved capacity.

##### Provisioned and PTUs

A Provisioned deployment reserves throughput measured in **Provisioned Throughput Units**, or **PTUs**.

Two separate conditions must be satisfied:

1. The subscription has sufficient **PTU quota**.
2. Azure has sufficient physical **capacity** for that model/version and deployment location.

PTU quota is only permission to deploy up to a limit. Having PTU quota does not guarantee that deployable physical capacity is available. Provisioned deployments support hourly billing and reservations for suitable sustained workloads. See [Provisioned throughput](https://learn.microsoft.com/en-us/azure/foundry/openai/concepts/provisioned-throughput) and [PTU billing](https://learn.microsoft.com/en-us/azure/foundry/openai/concepts/provisioned-throughput-billing).

> **Exam trap:** quota is not physical capacity, and PTU quota itself is not the provisioned deployment charge.

#### 4. Application scaling versus model scaling

| Symptom | Likely action |
|---|---|
| Application CPU or memory is saturated | Add or resize application instances |
| Application queue grows while model capacity is available | Scale the application tier |
| Model TPM reaches its limit | Increase or reallocate applicable TPM, or reduce token demand |
| Model RPM reaches its limit | Reduce request frequency or increase applicable RPM capacity |
| Provisioned deployment lacks sustained throughput | Add PTUs if quota and capacity are available |
| Agent repeatedly calls tools or models | Fix orchestration or apply loop limits |

Adding application instances does **not** increase a model deployment’s TPM or RPM.

It may make throttling worse because additional instances can send more concurrent requests to the same endpoint.

> **Exam rule:** scale the layer whose metrics show saturation.

#### 5. Handling HTTP 429

HTTP `429 Too Many Requests` means a rate or capacity limit was exceeded. It does not necessarily mean that the model is unavailable.

Correct handling:

1. Inspect TPM, RPM, concurrency and agent/service limits.
2. Honour `Retry-After` when supplied.
3. Otherwise use exponential backoff.
4. Add random jitter.
5. Limit the number of attempts.
6. Apply caller-side rate limiting.
7. Use idempotency protection for operations with side effects.

```text
Attempt 1 → short delay
Attempt 2 → longer delay + jitter
Attempt 3 → longer delay + jitter
Stop after a bounded number of attempts
```

Immediate retries from many clients create a **retry storm**, increasing pressure on the already throttled deployment. Microsoft recommends exponential backoff with jitter and bounded retries. See [Foundry retry guidance](https://learn.microsoft.com/en-us/azure/foundry/reference/foundry-project) and [Agent Service limits](https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/limits-quotas-regions).

#### 6. Back-pressure and bounded queues

**Back-pressure** means slowing, delaying or rejecting incoming work when a downstream component cannot process it safely.
...
- Controlled client retries

##### Why bounded queues matter

An unlimited queue does not solve overload. It converts throttling into:
...
When a bounded queue becomes full, the application should deliberately reject, delay or redirect work according to its design.
...
A local limiter on each application instance does not automatically protect a shared deployment.

Example:

```text
Model limit:                100 RPM
10 application instances:   20 RPM each
Possible total:             200 RPM
```

The combined traffic exceeds the endpoint limit.
...
- Coordinated allocation per instance

The limiter must account for the **aggregate traffic** reaching the deployment.

> **Exam trap:** adding instances multiplies local concurrency unless rate limits are coordinated.

#### 8. Regional capacity and failover

Regional failover requires an independently usable deployment path.

Verify for every destination:

- Model and version availability
- Supported deployment type
- Applicable TPM or PTU quota
- Deployable physical capacity
- Authentication and networking
- Routing and health checks
- Ability to handle failover traffic
...
Do not assume that capacity assigned to the primary deployment automatically guarantees secondary capacity.

For provisioned deployments, PTU quota and physical capacity can be region- and offering-specific. For newer shared Standard quota pools, the quota may be subscription- or data-zone-scoped, but regional model availability and runtime capacity must still be verified.

#### 9. Token usage, agent loops and cost

An agent request can generate multiple billable operations:
...
User request
  ...
  → retrieval
  → tool call
  → model interpretation call
  → another tool call
  → final model response
```

Therefore, one user request is not necessarily one model call.
...
- Use the smallest model that passes evaluation.
- Remove unnecessary prompt text.
- Retrieve fewer but more relevant chunks.
- Summarise or trim conversation history.
- Limit output length appropriately.
- Set maximum tool calls and agent iterations.
- Cache safe, reusable results.
- Use Batch for suitable asynchronous work.
- Reserve PTUs only when sustained utilisation justifies them.
- Remove or reduce unused quota allocations.
- Set Azure budgets and cost alerts.

> **Best metric:** cost per successful evaluated task—not merely cost per API call.

#### 10. Capacity-monitoring signals

| Signal | What it diagnoses |
|---|---|
| TPM utilisation | Token-capacity pressure |
| RPM utilisation | Request-frequency pressure |
| HTTP 429 count or rate | Throttling |
| Concurrent requests | In-flight workload pressure |
| Queue depth and wait time | Back-pressure and application capacity |
| Application CPU and memory | Application-tier saturation |
| End-to-end latency | User-observed performance |
| Time to first token | Initial model-response latency |
| Input and output tokens | Cost and capacity consumption |
| Calls per agent run | Loops and orchestration inefficiency |
| Tool failures and retries | External dependency problems |
| PTU utilisation | Provisioned-capacity efficiency |
| Cost per successful task | Economic efficiency |

Use percentiles such as p50, p95 and p99. An average can hide severe peak-period latency.

Quota influences admission, but achieved throughput also depends on request size, latency, concurrency and workload shape. See [Performance and latency guidance](https://learn.microsoft.com/en-us/azure/foundry/openai/how-to/latency).

#### 11. Troubleshooting workflow

Use this sequence:

1. **Locate the saturated layer:** application, agent, model or dependency.
2. **Identify the exact limit:** TPM, RPM, concurrency, PTUs, CPU, memory or queue.
3. **Determine the pattern:** temporary burst or sustained demand.
4. **Apply the matching correction.**
5. **Protect the service:** back-pressure and controlled retries.
6. **Load-test the change.**
7. **Monitor performance and cost.**

Examples:

```text
TPM full + unused applicable quota
→ increase or reallocate TPM

RPM full + TPM low
→ control request frequency or obtain RPM capacity

Application CPU full + model utilisation low
...
Temporary spike
→ bounded queue + backoff + jitter
```

#### 12. Scenario-to-solution mapping

| Scenario | Best answer |
|---|---|
| Large prompts trigger 429s | Investigate TPM |
| Many small requests trigger 429s | Investigate RPM |
| Applicable Standard quota is unused | Increase deployment TPM allocation |
| Another deployment holds unused applicable TPM | Reduce it and reallocate quota |
| Application CPU is 95%, model TPM is 40% | Scale application instances |
| Model TPM is 100%, application CPU is 30% | Increase or reallocate TPM, or reduce token demand |
| Ten application instances exceed one endpoint limit | Use coordinated distributed limiting |
| Short temporary traffic burst | Queue, apply back-pressure and retry safely |
| Sustained predictable production demand | Consider Provisioned throughput |
| Low unpredictable demand | Prefer an appropriate Standard deployment |
| Large asynchronous workload | Consider Batch |
| Agent cost rises with stable request count | Inspect context size, tool loops and calls per run |
| Regional failover is required | Prepare independent deployable capacity and routing |

#### 13. Common exam traps

- **“Add more application instances to fix model throttling.”** Application instances do not increase TPM or RPM.
- **“Every 429 means TPM is exhausted.”** RPM, concurrency or another service limit may be responsible.
- **“Retry immediately until the request succeeds.”** Immediate retries amplify throttling.
- **“An unlimited queue prevents request loss.”** It can cause unlimited memory growth and extreme latency.
- **“Each instance can independently use the endpoint’s full RPM.”** The endpoint limit applies to aggregate traffic.
- **“Increasing maximum output tokens increases capacity.”** It can increase estimated token demand and potential cost.
- **“Creating another Foundry project creates more quota.”** Projects do not bypass the applicable subscription quota pool.
- **“Having PTU quota guarantees deployment.”** Physical capacity must also be available.
- **“Provisioned is always better than Standard.”** Provisioned fits sustained predictable workloads; Standard often fits variable demand better.
- **“One regional deployment provides failover.”** Regional resilience requires another independently usable endpoint.
- **“One user request equals one model call.”** Agents can perform multiple model and tool calls per task.

#### Final memorisation summary

```text
TPM = token volume
RPM = request count
429 = throttled; find the exhausted limit

Standard = shared capacity, variable demand
Provisioned = reserved throughput measured in PTUs
Batch = asynchronous work

Quota = permission limit
Allocation = quota assigned to a deployment
Capacity = infrastructure actually available
Usage = work actually consumed

Application saturated → scale application
Model saturated → scale or reallocate model capacity
Temporary overload → back-pressure + bounded queue
Retry → Retry-After or exponential backoff + jitter
Multiple instances → distributed rate limiting
Failover → capacity and deployment ready in every target
Cost → optimise tokens, model calls, tool loops and successful-task cost
```

> **Best exam rule:** identify the constrained layer and exhausted unit first; then choose the smallest correction that directly addresses that constraint.
...
Monitor two different dimensions:

| Dimension | Main question |
|---|---|
| **Operational health** | Is the system available, fast and within capacity limits? |
| **AI quality and safety** | Is the result useful, correct, grounded and safe? |

An HTTP `200` response proves only that the request succeeded technically. The answer can still be irrelevant, unsupported, incomplete or unsafe.

Monitor the complete execution path:

```text
Input → retrieval → model → tools → final response
```
...
|---|---|
| Reliability | Request count, failures, timeouts, HTTP 429/5xx |
| Performance | End-to-end latency, time to first token, tool latency |
| Usage | Input/output tokens, model calls, tool calls, retries |
| Response quality | Relevance, coherence, completeness, task adherence |
| Grounding | Retrieval relevance, groundedness, citation correctness |
| Agent process | Tool selection, input accuracy, output usage, call success |
| Safety | Harm categories, jailbreaks, policy violations, unsafe actions |

Record model version, agent version, prompt version, scenario, region, timestamp and trace ID. Otherwise, you cannot reliably identify which change caused a regression.

#### Metrics, traces and evaluators

| Instrument | Best use |
|---|---|
| **Metrics and dashboards** | Detect aggregate trends and thresholds |
| **Logs** | Record discrete events and diagnostic details |
| **Traces** | Explain one complete request across models, retrieval and tools |
| **Evaluators** | Measure semantic quality and safety |
| **User feedback** | Detect real-world failures that automated metrics missed |

Foundry observability combines evaluation, production monitoring and OpenTelemetry-based tracing with Azure Monitor Application Insights. See [Microsoft Foundry observability](https://learn.microsoft.com/en-us/azure/foundry/concepts/observability).

> **Exam rule:** metrics reveal that something is wrong; traces help explain why; evaluators determine whether the AI result is good.

#### Baselines and drift

Create a representative **golden dataset** before production. Include:

- Normal scenarios
- Difficult edge cases
- Expected answers or outcomes
- RAG and citation cases
- Tool-call scenarios
- Safety and adversarial cases

A baseline records expected quality, safety, latency and cost for a known version.

**Drift** means that inputs, outputs or measured performance change over time. It is not necessarily a service exception.

Examples:

- Users begin asking different kinds of questions.
- Documents or permissions change.
- Retrieval relevance declines.
- Groundedness or task completion falls.
- Tool failures or agent loops increase.
- Safety events become more frequent.
- A model, prompt or agent version changes behaviour.

Compare results by scenario, model version, agent version and time period. A global average can hide a severe regression affecting one user journey.

#### Continuous evaluation

Use evaluation throughout the lifecycle:

1. **Before deployment:** evaluate against golden and safety datasets.
2. **During rollout:** canary the new version and compare it with the baseline.
3. **After deployment:** evaluate a lawful sample of production traffic.
4. **On a schedule:** rerun stable datasets to detect drift.
5. **After incidents:** add failed cases to the regression dataset.

Foundry supports continuous evaluation of sampled production traffic, scheduled evaluation datasets and alerts when quality or safety thresholds fail. See [Foundry evaluation lifecycle](https://learn.microsoft.com/en-us/azure/foundry/concepts/observability).
...
|---|---|
| **Retrieval relevance** | Did search return evidence relevant to the question? |
| **Groundedness** | Are response claims supported by the provided context? |
| **Response relevance** | Does the final response address the user’s question? |
| **Completeness** | Does the response include the necessary information? |
| **Citation correctness** | Do citations support the associated claims? |

A response can be grounded in an irrelevant document. Therefore, high groundedness alone does not prove that retrieval worked correctly.

```text
Bad retrieval + faithful use of bad context
= possibly high groundedness but poor answer relevance
```

For RAG incidents, inspect:

1. Retrieved documents and chunks
2. Query and filters
3. Retrieval and ranking scores
4. Groundedness
5. Response relevance and completeness
6. Citations

See [RAG evaluators](https://learn.microsoft.com/en-us/azure/foundry/concepts/evaluation-evaluators/rag-evaluators).

#### Agent evaluation

Evaluate both the final outcome and the execution process.

| Evaluation level | Examples |
|---|---|
| **System evaluation** | Task completion, task adherence, intent resolution, response quality |
| **Process evaluation** | Tool selection, tool inputs, tool-call success and use of tool output |

An agent may produce a plausible final answer after:

- Calling the wrong tool
- Passing incorrect arguments
- Ignoring the tool result
- Repeating unnecessary calls
- Attempting an unauthorised action

Monitoring only the final response can hide these failures. See [Foundry agent evaluators](https://learn.microsoft.com/en-us/azure/foundry/concepts/evaluation-evaluators/agent-evaluators).

#### Safety-event monitoring

For every safety event, capture:

- Category and severity
- Input or output classification
- Model, prompt and agent version
- Tool actions proposed or executed
- Trace and request IDs
- Timestamp and affected scenario

Then:

1. Inspect the complete trace.
2. Determine whether the cause was user input, retrieved content, prompt, model output or tool orchestration.
3. Adjust filters, instructions, retrieval, permissions or approval controls.
4. Add the incident to the safety regression dataset.
5. Re-evaluate before redeployment.

A content-filter event is a signal, not a complete diagnosis. Safety monitoring should also include jailbreaks, indirect prompt injection, unsafe tool usage and policy violations.

#### Troubleshooting workflow

```text
Detect regression
→ segment by version and scenario
→ inspect metrics and traces
→ identify failing pipeline stage
→ add regression test
→ fix responsible component
→ evaluate offline
→ canary
...
- Source data
- Tool schema or implementation
- Conversation state
- Agent orchestration
- Identity or permissions
- Safety configuration

Do not automatically replace the model before identifying the failing component.

#### Tracing and privacy

OpenTelemetry traces can connect:
...
Full prompts, responses and tool payloads improve diagnosis but can contain personal, confidential or regulated data.
...
- Sampling
- Lawful production-data handling

Keep structured metadata and trace IDs even when payload text cannot be stored. See [Foundry tracing and data handling](https://learn.microsoft.com/en-us/azure/foundry/observability/concepts/trace-data).

#### High-value exam traps

- **HTTP 200 means the answer is correct.** It proves only technical request success.
- **Low latency means good model performance.** Speed does not prove relevance, safety or groundedness.
- **High groundedness means retrieval was correct.** The model may faithfully use irrelevant evidence.
- **Monitor only the final response.** Agent failures can occur during tool selection, arguments or orchestration.
- **Use traces to measure aggregate drift.** Traces explain individual runs; metrics and evaluations detect trends.
- **Drift means the model endpoint failed.** Drift is changing data or measured behaviour over time.
- **Fix every quality problem by changing the model.** The actual problem may be retrieval, prompts, tools, state or permissions.
- **Store every production prompt for diagnosis.** Full tracing introduces privacy, security, retention and cost risks.

#### Final memorisation

```text
Metrics    = detect trends
Logs       = record events
Traces     = explain one execution
Evaluators = measure semantic quality and safety

Baseline   = known expected performance
Drift      = performance or behaviour changes over time

Retrieval relevance = was the evidence appropriate?
Groundedness        = was the answer supported by that evidence?
Response relevance  = did the answer address the question?

System evaluation  = final outcome
Process evaluation = agent steps and tool use
```

> **Best exam rule:** baseline the system, monitor operational and semantic signals, evaluate retrieval and groundedness separately, trace the entire agent path, and convert every important failure into a regression test.
