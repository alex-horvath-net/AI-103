...
- **Operational performance:** Is the system available, fast and within capacity and cost limits?
- **AI quality:** Are results relevant, grounded, safe and useful?

An HTTP 200 response proves only that the service answered. It does not prove task success. Monitor the whole path: input → retrieval → model → tools → output.

#### Signals to monitor

| Area | Signals | What it reveals |
|---|---|---|
| Availability | Requests, errors, timeouts, 429s and 5xx responses | Whether calls complete reliably |
| Latency | End-to-end latency, time to first token and time between tokens | User experience and slow stages |
| Throughput | Requests, tokens and tool calls over time | Capacity pressure and workload growth |
| Cost | Input/output tokens, model calls and evaluator calls | Cost per request and per successful task |
| Quality | Relevance, coherence, task adherence and groundedness | Whether the response solves the task |
| Safety | Safety categories, policy violations and unsafe tool proposals | Whether content and actions are acceptable |
| Agent process | Retrieval quality, tool selection/input/output, retries and loops | Whether intermediate steps work correctly |

Record model name/version, agent version, scenario, region, request or trace ID and timestamp. These dimensions make regressions comparable.

#### Baselines and drift

Create a representative **golden dataset** and a separate **safety dataset** before changing the model, prompt, retrieval pipeline or tools. Record the baseline for quality, latency, token use, cost, safety and task completion.

**Drift** is a change over time in inputs, outputs or measured behaviour. Examples include:

- users ask different questions;
- source documents or permissions change;
- relevance or groundedness declines;
- latency, token use, retries or tool failures increase;
- safety-event frequency changes.

Segment results by version, scenario, tenant where appropriate and time window. A global average can hide one failing scenario.

#### Grounding quality

Groundedness asks: **Are the answer’s claims supported by the supplied evidence?**

Measure two related but different properties:

1. **Retrieval quality:** Did search return relevant and sufficient evidence?
2. **Answer groundedness:** Did the model use that evidence accurately and avoid unsupported claims?

An answer can be grounded in irrelevant documents, so a groundedness score alone does not prove retrieval quality. Preserve citations, source IDs and page/chunk references where lawful.

#### Evaluation and error analysis

Use the right instrument for the question:

- **Metrics and dashboards** reveal aggregate trends.
- **Traces** explain one request’s retrieval, model, tool and timing path.
- **Evaluators and golden datasets** measure semantic quality such as relevance, groundedness, task adherence and tool-call accuracy.
- **Safety evaluations** detect harmful or policy-sensitive behaviour.

A practical incident loop is:

1. Alert on a threshold or trend.
2. Segment the failure by version, scenario and pipeline stage.
3. Inspect the trace and retrieved evidence.
4. Classify the cause: model, prompt, retrieval, tool, state, orchestration or permission.
5. Add the case to a regression dataset.
6. Fix the responsible component.
7. Re-evaluate offline, then canary and compare with the baseline.
8. Roll back when quality, safety or service-level thresholds regress.

#### Tracing and privacy

Foundry tracing and Azure Monitor/Application Insights can connect model calls, retrieval, tools, retries, latency and final outcomes. Full payload traces help diagnose failures but increase privacy, retention and cost risk. Apply data minimisation, redaction, protected access and retention policies. Keep structured metadata even when sensitive prompt or response text is omitted.

#### Exam traps

- HTTP 200 means the service responded; it does not prove relevance, groundedness or task completion.
- Low latency does not prove quality.
- Groundedness does not prove that retrieval returned the right documents.
- Token count alone does not explain an agent loop or tool failure.
- Aggregate metrics show trends; traces explain individual failures; evaluators measure semantics.
- Drift is not only a model error; it is changing data or measured behaviour over time.
- Monitoring only the final answer can hide unsafe or wasteful intermediate tool calls.
- Full tracing is not automatically correct: protect sensitive data and control retention.
- Fixing the model is not always the answer; first identify whether the fault is prompt, retrieval, tool, state, orchestration or permission.

#### Exam summary

**Baseline versioned quality, monitor operational and semantic signals, evaluate retrieval and groundedness separately, trace the complete agent path, and turn incidents into regression tests before promotion.**
