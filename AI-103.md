...
`OpenAI` is the required resource kind for generating content from prompts. `--assign-identity` assigns an identity that the Cognitive Services resource can use to access the customer-managed key in Azure Key Vault.
...
- **C. `--encryption`:** Represents encryption configuration rather than the identity-assignment option selected in the picture. The picture separately shows the Key Vault encryption JSON.
