...
> Conversation state keeps the current dialogue. Long-term memory keeps selected durable context. Knowledge supplies external evidence. Tools perform bounded operations. An agent orchestrates these parts.

| Component | Exam meaning | Typical choice |
|---|---|---|
| **Conversation state** | Messages and task context for the current conversation | Continue a multi-turn task |
| **Long-term memory** | Selected facts retained in a separate memory store | Remember a user preference across sessions |
| **Knowledge** | External or organisational information used for grounding | Search current or private documents |
| **Tool** | A bounded capability that retrieves, calculates, or acts | Function, OpenAPI, MCP, or Code Interpreter |
| **Agent** | Model plus instructions, state, knowledge, tools, and safeguards | Decide which steps and tools are needed |

**Conversation state versus long-term memory**

| Requirement | Choose | Exam distinction |
|---|---|---|
| Continue the current conversation | Conversation/thread state | Limited by the available context; do not assume an old long conversation is fully present |
| Remember selected facts across sessions or devices | Foundry memory | Separate from the thread; retrieves relevant memory items, not a complete transcript |
| Ground an answer in business documents | Knowledge integration | Provides source evidence; it does not retrain the model |
| Perform an operation | Tool | The model may request it, but authorisation and execution remain outside the model |

Foundry memory is a managed long-term-memory capability and is currently documented as preview. When enabled, configure the memory scope, retention or expiry, update and delete behaviour, and protection of sensitive data. Memory and conversation state have different lifecycles.

**Knowledge integration**

| Scenario | Best starting choice | Key exam point |
|---|---|---|
| Search a small set of uploaded files | **File Search** | Managed file retrieval for the agent |
| Control indexes, fields, filters, and retrieval logic | **Azure AI Search** | Supports full-text, vector, hybrid, and semantic ranking |
| Complex conversational retrieval | **Azure AI Search agentic retrieval** | A knowledge base can plan or decompose queries, retrieve in parallel, rerank, and return grounding data |
| Reusable, multi-source, permission-aware knowledge for several agents | **Foundry IQ** | Managed knowledge layer built on Azure AI Search capabilities |
| Current public information | **Web Search** | Use when freshness matters |

Classic Azure AI Search retrieval normally leaves query orchestration and grounding-prompt construction to the application. Current agentic retrieval adds a knowledge-base pipeline that handles more of that retrieval work, but the calling application or agent still invokes it and uses the returned grounding data.

**Tool selection**

| Requirement | Choose | What to remember |
|---|---|---|
| Call application code | **Function tool** | The model requests a structured call; the application executes it and returns the result |
| Call a REST API described by YAML or JSON | **OpenAPI tool** | The specification describes operations, parameters, schemas, and security; credentials come from secure configuration |
| Use tools exposed by a remote standardised server | **MCP tool** | MCP standardises tool and context exchange; it does not replace the model or agent |
| Analyse files, calculate, or create charts | **Code Interpreter** | The model can write and run code in a sandbox; it is not a generic business-API connector |

**Typical runtime**

Request → current conversation state → relevant memory or knowledge retrieval → model chooses whether to answer or call a tool → identity, RBAC, and approval checks → retrieval or tool execution → model produces the grounded response → optionally update memory.

**High-value exam traps**

- **State is not memory:** state supports the current dialogue; memory preserves selected facts beyond it.
- **Memory is not knowledge:** memory personalises continuity; knowledge grounds answers in external sources.
- **Search is not the final answer:** retrieval supplies evidence; the model generates the response.
- **Vector search is not semantic ranking:** vector search compares embeddings; semantic ranking reranks an initial result set; hybrid combines lexical and vector retrieval.
- **Function calling is not execution:** your application must execute the requested function and return its output.
- **OpenAPI is not a secret store:** the document describes the API; use a secure connection for credentials.
- **MCP is not an LLM:** it exposes tools or context; the model still reasons and selects calls.
- **Tool selection is not authorisation:** enforce permissions in the target service and require approval for consequential actions.
- Treat retrieved documents and tool output as untrusted data because they can contain indirect prompt injection.
- Updating a knowledge source does not retrain the model; it changes what can be retrieved at run time.

**Repeatable exam workflow**

1. Identify the required outcome: remember, know, retrieve, calculate, or act.
2. Select the narrowest matching state, memory, knowledge source, or tool.
3. Check modality, freshness, complexity, latency, throughput, context, cost, security, and data-residency requirements.
4. Verify identity, RBAC, network path, tool scope, and approval requirements.
5. Check current Microsoft documentation for availability, API version, and preview status; then evaluate with representative data.

**Exam summary**

> State = current conversation. Memory = selected durable context. Knowledge = external evidence. Search = retrieval. Tool = bounded operation. Agent = orchestration. Model = reasoning or generation. Authorisation = application and service responsibility.

**Official sources**

- [Memory in Microsoft Foundry Agent Service](https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/what-is-memory)
...
- [OpenAPI tools](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/openapi)
...
Check the linked documentation before the exam because preview status, supported regions, API versions, and service behaviour can change.
#### Quize
