...
Start by deciding **what capability the workload needs**, then choose the smallest option that meets the quality and operational requirements.

| Capability | What it does | Choose it when |
|---|---|---|
| Large language model (LLM) | Reasons over and generates language; some LLMs also accept other modalities | The task needs strong reasoning, nuanced generation, complex instructions, coding, or synthesis |
| Small language model (SLM) | Performs language tasks with a smaller model | The task is bounded and repeatable, and latency, throughput, cost, or constrained hosting matters more than maximum capability |
| Multimodal model | Reasons across supported combinations of text, images, audio, or video | The answer must combine meaning across modalities, such as answering a question about an image |
| Embedding model | Converts content into numeric vectors that preserve semantic similarity | The application must retrieve, rank, cluster, or compare content; it does **not** generate the final answer |
| Image or video model | Generates or edits visual media | The required output is an image or video, rather than an explanation of existing media |
| Foundry Tool | Performs a specialised, bounded capability through a purpose-built service | The requirement explicitly asks for speech, translation, structured content extraction, OCR/vision, or another supported specialist operation |
| Agent | Uses a model with instructions, knowledge, and tools to complete a goal | The solution must choose actions, call tools, use external data, or continue across multiple steps |

The distinction to memorise is:

- A **model** reasons, predicts, embeds, or generates.
- A **Foundry Tool** performs a specialised capability with a defined interface.
- An **agent** orchestrates a model, instructions, knowledge, and tools. An agent is not itself a model.

#### Choose in the right order

1. **Identify the modality and output.** Is the input text, image, audio, video, or a document? Is the output text, vectors, structured fields, speech, an image, or an action?
2. **Classify the task.** Decide whether it is generation, summarisation, extraction, classification, retrieval, translation, perception, or multistep action.
3. **Prefer a bounded service for a bounded requirement.** Use a specialised Foundry Tool when the exam asks for an established capability such as speech recognition, translation, OCR, or schema-based extraction. Use a generative model when flexible reasoning or generation is central.
4. **Add grounding only when needed.** For current, private, or domain-specific knowledge, retrieve relevant content and provide it to the generative model. Embeddings plus Azure AI Search are common RAG components; a larger model alone does not make private data available.
5. **Add an agent only when orchestration is required.** A single prompt-response call does not automatically require an agent.
6. **Apply constraints.** Compare quality, latency, throughput, context size, cost, safety, deployment type, hosting effort, regional availability, and inferencing residency.
7. **Evaluate with representative data.** Start with the smallest and least costly candidate likely to succeed, then move to a larger model only when measured quality is insufficient.

#### Task-to-capability decisions

| Exam scenario | Best starting choice | Why |
|---|---|---|
| Draft, rewrite, summarise, explain, or answer complex questions | Language model; LLM for harder reasoning, SLM for bounded workloads | The required output is generated language |
| High-volume intent classification or simple structured extraction | SLM or an appropriate specialised language/content tool | Maximum general reasoning may not justify the added latency and cost |
| Extract named fields from invoices, contracts, calls, images, audio, or video | Content Understanding | It converts unstructured or multimodal content into a defined output schema and can provide confidence and grounding information |
| Retrieve semantically similar passages | Embedding model plus a vector index in Azure AI Search | Embeddings represent similarity; Search stores and retrieves matching content |
| Answer from private documents with citations | Retrieval pipeline plus a generative model (RAG) | Retrieval supplies the evidence; the model composes the grounded answer |
| Ask an open-ended question about an image | A model whose current model card confirms image input | The task requires flexible reasoning over both the prompt and image |
| OCR, object/face analysis, or another explicit vision feature | The currently supported Vision or Content Understanding capability | A bounded visual operation does not necessarily need a general multimodal model; check current retirement guidance |
| Speech recognition, speech synthesis, or speech translation | Speech | Speech provides purpose-built audio and voice capabilities |
| Translate text or documents | Translator | Translation is the explicit bounded requirement, including document-preserving workflows |
| Generate or edit an image or video | A suitable image/video generation model | Understanding models analyse media; generation models produce it |
| Choose tools and execute a multistep business process | Agent with an appropriate model, knowledge source, and least-privilege tools | The requirement is orchestration and action, not only text generation |

#### Large versus small language models

| Consideration | Larger model | Smaller model |
|---|---|---|
| Quality and reasoning | Usually the stronger starting point for ambiguous, novel, or multistep tasks | Often sufficient for narrow classification, extraction, rewriting, and simple generation |
| Latency and throughput | Can require more compute and may respond more slowly | Often better suited to low-latency and high-volume workloads |
| Cost | Generally higher consumption or hosting cost | Generally lower cost per workload when it meets the quality target |
| Hosting and operations | Managed deployment reduces infrastructure work, but model and deployment choices still affect quotas and performance | Some SLMs can suit constrained or local hosting, but self-hosting adds patching, scaling, monitoring, and capacity management |
| Context and features | Do not infer capability from size alone | Do not assume a smaller model has a smaller context window; verify the model card |

Model size is a **trade-off**, not an automatic answer. Public benchmarks and the Foundry model leaderboard help shortlist candidates, but Microsoft recommends evaluation against your own representative data. Compare task quality and safety together with estimated cost and throughput.

#### Multimodal model or specialised Foundry Tool?

Use a multimodal model when the requirement is **open-ended reasoning across modalities**: for example, “compare this chart with the written policy and explain the discrepancy”.

Use a specialised tool when the requirement is **well-defined and operational**:

- **Speech** for transcription, text to speech, and supported speech translation.
- **Translator** for text or document translation, especially when document structure or translation controls matter.
- **Content Understanding** for schema-based extraction and classification across documents, images, audio, and video.
- **Vision or another currently supported visual capability** for a documented bounded visual feature.
- **Azure AI Search** for keyword, vector, or hybrid retrieval; it retrieves evidence rather than writing the final response.

A specialised service can provide a more predictable contract, metadata, confidence values, or domain-specific controls. A multimodal generative model offers greater flexibility but must be evaluated for output reliability. These capabilities can be combined: for example, Content Understanding can prepare structured content, Search can retrieve it, and an LLM can generate a grounded answer.

#### Compact examples

- **Generation:** create a customer email from case notes → language model.
- **Summarisation:** summarise a complex legal comparison → begin with an LLM; test an SLM if the format and domain are stable.
- **Extraction:** return fixed invoice fields with confidence information → Content Understanding.
- **Classification:** route thousands of short support messages → evaluate an SLM or specialised classifier.
- **RAG:** answer from internal policies → embedding/vectorisation + Azure AI Search + a generative model.
- **Image understanding:** explain why a machine part in a photograph appears damaged → multimodal model supporting image input.
- **Speech:** transcribe a call and synthesise a spoken reply → Speech.
- **Translation:** translate documents while preserving structure → Translator document translation.
- **Embeddings:** find semantically related cases → embedding model + vector search.
- **Agent tool use:** check inventory, request approval, and place an order → agent with authorised inventory, approval, and ordering tools.

#### Constraints that can change the answer

- **Region and availability:** model, feature, deployment type, quota, and service availability change. Verify the current Foundry model catalogue, model card, and region-support documentation.
- **Inferencing residency:** this is where prompts and generated outputs are processed during inference. It is not the same as where stored data rests. Deployment type can determine whether inference is processed in one Azure region or within a broader data zone.
- **Safety:** evaluate harmful-content risk, grounding, prompt injection exposure, tool permissions, and required guardrails. A high benchmark score does not prove workload safety.
- **Deployment:** compare serverless/managed consumption with provisioned or self-hosted options where available. Reserved capacity can improve predictable throughput; self-hosting increases operational responsibility.
- **Context size:** count instructions, conversation history, retrieved passages, tool results, input media representations, and desired output. Never select from the advertised context window alone; validate effective quality on realistic long inputs.
- **Tool compatibility:** if an agent must use function calling or structured output, confirm that the selected model and deployment support the required feature.

#### Common exam traps

- **“Agent” is not the answer merely because the application is conversational.** Choose an agent when it must orchestrate knowledge, tools, state, or multistep actions.
- **An embedding model does not answer questions.** It creates vectors used by retrieval; a generative model normally produces the natural-language RAG answer.
- **Multimodal understanding is not media generation.** Analysing an image and creating an image are different model capabilities.
- **A file is not automatically a multimodal requirement.** Plain text extracted from a file can be processed by a language model; select multimodal processing only when layout, imagery, audio, or video meaning matters.
- **The largest model is not automatically best.** Select the least costly option that passes the required quality, safety, latency, and throughput tests.
- **A larger context window is not a substitute for retrieval.** Use Search/RAG when the workload requires discoverable, current, private, or citable knowledge.
- **Do not use a general model when the question explicitly requires a bounded service capability.** Speech, Translator, Content Understanding, Vision, and Search have distinct roles.
- **Do not memorise a volatile model list.** Confirm current modalities, features, regions, quotas, deployment types, and lifecycle status in official documentation.

#### Repeatable exam workflow

For every unfamiliar scenario, ask:

1. What goes in, and what must come out?
2. Is the task bounded or does it require flexible reasoning?
3. Does it need external/private knowledge?
4. Does it need tool calls or multiple steps?
5. What quality, latency, throughput, context, cost, safety, region, residency, and hosting constraints apply?
6. Which smallest supported candidate passes an evaluation using representative inputs?

#### Exam summary

Memorise these boundaries: **LLM/SLM generate and reason; multimodal models reason across supported media; embedding models create vectors; image/video models generate media; Foundry Tools perform specialised bounded operations; Azure AI Search retrieves knowledge; agents orchestrate models, knowledge, and tools.**

Choose by modality and task first, then validate quality, safety, latency, throughput, context, cost, deployment, region, and inferencing residency. Use current catalogue and region documentation instead of relying on remembered model names or availability.

#### Sources

- [Microsoft Foundry overview](https://learn.microsoft.com/en-us/azure/foundry/what-is-foundry)
- [Model benchmarks in the Foundry model catalogue](https://learn.microsoft.com/en-us/azure/foundry/concepts/model-benchmarks)
- [Model leaderboard and comparison guidance](https://learn.microsoft.com/en-us/azure/foundry/how-to/benchmark-model-in-catalog)
...
- [Azure Vision overview and lifecycle guidance](https://learn.microsoft.com/en-us/azure/ai-services/computer-vision/overview)
