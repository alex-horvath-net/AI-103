...
**Why:** `AzureKeyVault` identifies the connection target as the existing Azure Key Vault resource `KV1`. `AccountManagedIdentity` uses the Foundry account or project managed identity instead of storing a Key Vault key or API key. The managed identity must still have the required Key Vault RBAC permissions.
...
- `AccountKey` and `ApiKey` use secret-based credentials rather than the required managed identity.
