...
Plan subscription/resource group, region, Foundry resource, projects, model deployments, app/agent compute, data stores, Azure AI Search, Key Vault if secrets remain, Application Insights/Azure Monitor, private endpoints/DNS, identities, and RBAC.


*Official exercise visual: the project home page separates the project endpoint from the Azure OpenAI endpoint and provides entry points for model and agent work. Source: [Prepare for an AI development project](https://github.com/microsoftlearning/mslearn-ai-studio/blob/main/Instructions/Exercises/01-Explore-ai-studio.md#explore-microsoft-foundry).*

#### How it works and decision criteria

Region affects feature/model availability, quota, latency, inferencing residency, and paired services. Co-locate dependent services where practical. Prompt agents are platform-run definitions; hosted agents run your framework/custom code on managed infrastructure. External agents can remain elsewhere and be registered for observability.

#### Workflow and practice

Requirements and data classification → region/feature check → threat model → resource/project boundaries → identity/RBAC → network topology → models/data/tools → monitoring/evaluation → IaC and deployment pipeline → load/safety/recovery tests.

#### Decision criteria and trade-offs

Shared resources reduce cost/administration but enlarge blast radius. Separate subscriptions/resource groups improve isolation and chargeback. Bring-your-own storage/search/Cosmos DB when compliance requires control; Microsoft-managed dependencies reduce operations.


#### Boundary decisions

| Boundary                       | Share when                                            | Separate when                                                                   |
| ------------------------------ | ----------------------------------------------------- | ------------------------------------------------------------------------------- |
| Foundry resource or project    | Teams share governance, lifecycle, region, and access | Tenants, environments, data sensitivity, ownership, or release cadence differ   |
| Subscription or resource group | Central operations and chargeback are sufficient      | Quota, policy, billing, or blast-radius isolation is required                   |
| Network                        | Public access with identity controls meets policy     | Data or endpoints must remain on private address paths                          |
| Data and search                | Managed dependencies meet retention and control needs | The organization must own storage, encryption, networking, schema, or lifecycle |

The architecture is complete only when a request can be traced from caller identity through model, data, and tool permissions to monitoring and recovery.

> **Exam Trap**
>
> Project endpoint and Azure OpenAI endpoint serve different surfaces. Private networking controls reachability; RBAC still controls authorization.

#### Exam Summary

Design the control, identity, network, data, runtime, and observability planes together—not as post-deployment add-ons.

#### Sources

[What is Foundry](https://learn.microsoft.com/en-us/azure/foundry/what-is-foundry), [create projects](https://learn.microsoft.com/en-us/azure/foundry/how-to/create-projects), [region support](https://learn.microsoft.com/en-us/azure/foundry/reference/region-support)
...
**Full answer-area transcription from the supplied picture**

The picture shows the following Bicep content. The line break inside the API version is preserved from the picture's display:
...
**Dropdown selections shown in the picture:**

| Answer-area field | Selected value |
|---|---|
| `category` | `'AzureKeyVault'` |
| `authType` | `'AccountManagedIdentity'` |

**Visible values and metadata:**

- Existing resource type and API version: `Microsoft.KeyVault/vaults@2024-11-01`
- Existing resource name: `'KV1'`
- Scope: `resourceGroup()`
- Connection resource type and API version: `Microsoft.CognitiveServices/accounts/connections@2025-04-01-preview`
- Connection name: `'${aiFoundryName}-keyvault'`
- Parent: `aiFoundry`
- Target: `existingKeyVault.id`
- `ApiType`: `'Azure'`
- `ResourceId`: `existingKeyVault.id`
...
The supplied picture displays the property name `isSharedToAll:`, but no value is visible after the colon. No value has been invented here.

**Why the selected values are correct:** `AzureKeyVault` identifies the connection target as the existing Azure Key Vault resource `KV1`. `AccountManagedIdentity` uses the Foundry account or project managed identity rather than storing a Key Vault key or API key. The managed identity must still have the required Key Vault RBAC permissions.

**Why the other dropdown options are incorrect:**

- `AzureAIService` and `AzureOpenAI` identify different connection categories; they do not represent an Azure Key Vault connection.
- `AccountKey` and `ApiKey` use secret-based credentials rather than the managed identity shown in the picture.
